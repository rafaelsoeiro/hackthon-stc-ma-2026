    ...
    for (let j = 0; j < 10; j++) {
      ...
      obras.push({
        descricao: pick([
          ...
          'Reforma de hospital municipal',
          'Implantacao de drenagem pluvial',
          'Requalificacao de mercado municipal',
          'Ampliacao de rede de saneamento',
        ]),
        status: pick(['em_andamento', 'concluida', 'paralisada']),
        percentualExecucao: Number(rand(5, 100).toFixed(2)),
        dataInicio: new Date(`${2020 + Math.floor(Math.random() * 7)}-01-15`),
        dataPrevisaoFim: new Date(`${2020 + Math.floor(Math.random() * 7)}-12-20`),
